...
Begin VB.UserControl ctlVTKKaiSel 
   BackColor       =   &H00E0EEEE&
   ClientHeight    =   5565
   ClientLeft      =   0
   ClientTop       =   0
   ...
   Begin VB.Frame fraTop 
      ...
      Begin VB.ComboBox cmbJyoCD 
         Height          =   300
         ItemData        =   "ctlVTKKaiSel.ctx":0000
         Left            =   1590
         List            =   "ctlVTKKaiSel.ctx":0026
   ...
   Begin TabDlg.SSTab mstTab 
      ...
      TabHeight       =   520
      BackColor       =   14741230
      TabCaption(0)   =   "特別登録馬 開催一覧"
      TabPicture(0)   =   "ctlVTKKaiSel.ctx":006C
   ...
   End
End
Attribute VB_Name = "ctlVTKKaiSel"
...
Private WithEvents mData As clsDataTKKaiSel '' データ取得オブジェクト
...
On Error GoTo ErrorHandler
    ...
    ElseIf (KeyAscii < Asc("0") Or KeyAscii > Asc("9")) And Not KeyAscii = 8 Then
        ...
        Beep            ' エラー音を鳴らします。
    End If
    cmbYear.Text = Left(cmbYear.Text, 4)
    Exit Sub
ErrorHandler:
    gApp.ErrLog
End Sub
...
On Error GoTo ErrorHandler
    ...
    Set mData = New clsDataTKKaiSel '' データ取得オブジェクト
    Set mViewerState = New clsVSYearJyo

    gApp.InitLog Me
    mstrTitle = "特別登録馬 開催選択"
