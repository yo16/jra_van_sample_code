/*=======================================================================
  JRA-VAN Data Lab.プログラミングパーツ「データ登録パーツ　O4レコード登録クラス」
...
int clsImportO4::Add(CString strBuff, long lngBuffSize)
{
	CString strMakeDate;
...
int clsImportO4::InsertDB(void)
{

	int i;
	CString strsql;

	strsql="SELECT * FROM ODDS_UMATAN_HEAD WHERE [Year]='";
...
try{
	...
	lArrayIndex[0]=12;
	vaFieldlist.PutElement(lArrayIndex, &(_variant_t("UmatanFlag")));	
	strsql.SetString(mBuf.UmatanFlag,1);
	vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));

	lArrayIndex[0]=13;
	vaFieldlist.PutElement(lArrayIndex, &(_variant_t("TotalHyosuUmatan")));	
	strsql.SetString(mBuf.TotalHyosuUmatan,11);
	pRecordSet->AddNew(vaFieldlist, vaValuelist);
	pRecordSet->Close();	

	if(mBuf.UmatanFlag[0]!='0'){
				strsql="SELECT * FROM ODDS_UMATAN WHERE [Year] IS NULL";
				bstrQuery=strsql;
		...
		for(i=0;i<306;i++){
			if(strncmp(mBuf.OddsUmatanInfo[i].Kumi,"    ",4)!=0){
				...
				strsql.SetString(mBuf.OddsUmatanInfo[i].Kumi,4);
				vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));

				lArrayIndex[0]=8;
				vaFieldlist.PutElement(lArrayIndex, &(_variant_t("Odds")));	
				strsql.SetString(mBuf.OddsUmatanInfo[i].Odds,6);
				vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));

				lArrayIndex[0]=9;
				vaFieldlist.PutElement(lArrayIndex, &(_variant_t("Ninki")));	
				strsql.SetString(mBuf.OddsUmatanInfo[i].Ninki,3);
				vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));
				pRecordSet->AddNew(vaFieldlist, vaValuelist);
			}
		}
...
int clsImportO4::UpdateDB(CString strMakeDate)
{
...
int clsImportO4::Init(_ConnectionPtr m_Connect)
{
    pCn = m_Connect;
