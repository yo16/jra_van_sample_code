/*=======================================================================
  JRA-VAN Data Lab.プログラミングパーツ「データ登録パーツ　O2レコード登録クラス」
...
int clsImportO2::Add(CString strBuff, long lngBuffSize)
{
	CString strMakeDate;
...
int clsImportO2::InsertDB(void)
{

	int i;
	CString strsql;

	strsql="SELECT * FROM ODDS_UMAREN_HEAD WHERE [Year]='";
...
try{
	...
	lArrayIndex[0]=12;
	vaFieldlist.PutElement(lArrayIndex, &(_variant_t("UmarenFlag")));	
	strsql.SetString(mBuf.UmarenFlag,1);
	vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));

	lArrayIndex[0]=13;
	vaFieldlist.PutElement(lArrayIndex, &(_variant_t("TotalHyosuUmaren")));	
	strsql.SetString(mBuf.TotalHyosuUmaren,11);
	pRecordSet->AddNew(vaFieldlist, vaValuelist);
	pRecordSet->Close();	


	if(mBuf.UmarenFlag[0]!='0'){
		strsql="SELECT * FROM ODDS_UMAREN WHERE [Year] IS NULL";
		bstrQuery=strsql;
		...
		for(i=0;i<153;i++){
			if(strncmp(mBuf.OddsUmarenInfo[i].Kumi,"    ",4)!=0){
				...
				strsql.SetString(mBuf.OddsUmarenInfo[i].Kumi,4);
				vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));

				lArrayIndex[0]=8;
				vaFieldlist.PutElement(lArrayIndex, &(_variant_t("Odds")));	
				strsql.SetString(mBuf.OddsUmarenInfo[i].Odds,6);
				vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));

				lArrayIndex[0]=9;
				vaFieldlist.PutElement(lArrayIndex, &(_variant_t("Ninki")));	
				strsql.SetString(mBuf.OddsUmarenInfo[i].Ninki,3);
				vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));
				pRecordSet->AddNew(vaFieldlist, vaValuelist);
			}
		}
...
int clsImportO2::UpdateDB(CString strMakeDate)
{
...
try{
	...
	strsql = "DELETE FROM ODDS_UMAREN";
...
int clsImportO2::Init(_ConnectionPtr m_Connect)
{
    pCn = m_Connect;
