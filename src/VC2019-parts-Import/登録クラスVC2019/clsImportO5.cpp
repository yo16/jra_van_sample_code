/*=======================================================================
  JRA-VAN Data Lab.プログラミングパーツ「データ登録パーツ　O5レコード登録クラス」
...
int clsImportO5::Add(CString strBuff, long lngBuffSize)
{
	CString strMakeDate;
...
int clsImportO5::InsertDB(void)
{

	int i;
	CString strsql;

	strsql="SELECT * FROM ODDS_SANREN_HEAD WHERE [Year]='";
...
try{
	...
	lArrayIndex[0]=12;
	vaFieldlist.PutElement(lArrayIndex, &(_variant_t("SanrenFlag")));	
	strsql.SetString(mBuf.SanrenpukuFlag,1);
	vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));

	lArrayIndex[0]=13;
	vaFieldlist.PutElement(lArrayIndex, &(_variant_t("TotalHyosuSanren")));	
	strsql.SetString(mBuf.TotalHyosuSanrenpuku,11);
	vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));
	pRecordSet->AddNew(vaFieldlist, vaValuelist);
	pRecordSet->Close();	

	if(mBuf.SanrenpukuFlag[0]!='0'){
				strsql="SELECT * FROM ODDS_SANREN WHERE [Year] IS NULL";
				bstrQuery=strsql;
		...
		for(i=0;i<816;i++){
			if(strncmp(mBuf.OddsSanrenInfo[i].Kumi,"      ",6)!=0){
				...
				strsql.SetString(mBuf.OddsSanrenInfo[i].Kumi,6);
				vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));
				lArrayIndex[0]=8;
				vaFieldlist.PutElement(lArrayIndex, &(_variant_t("Odds")));	
				strsql.SetString(mBuf.OddsSanrenInfo[i].Odds,6);
				vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));

				lArrayIndex[0]=9;
				vaFieldlist.PutElement(lArrayIndex, &(_variant_t("Ninki")));	
				strsql.SetString(mBuf.OddsSanrenInfo[i].Ninki,3);
				vaValuelist.PutElement(lArrayIndex, &(_variant_t(T2OLE(strsql))));
				pRecordSet->AddNew(vaFieldlist, vaValuelist);
			}
		}
...
int clsImportO5::UpdateDB(CString strMakeDate)
{
...
int clsImportO5::Init(_ConnectionPtr m_Connect)
{
    pCn = m_Connect;
	 return 0;
}
