...
Friend Class clsImportO5
	' @(h) clsReadO5.cls
	' @(s)
	' JVData "O5" データベース登録クラス
	'
	
	Private mBuf As JV_O5_ODDS_SANREN ''オッズ（3連複）構造体
    ...
    Private Sub Class_Initialize_Renamed()
        ...
        'レコードセットオープン
        strSql = "SELECT * FROM ODDS_SANREN_HEAD"
        ...
        'レコードセットオープン
        strSql = "SELECT * FROM ODDS_SANREN"
...
ErrorHandler:
    ...
    Public Function InsertDB() As Boolean
        ...
        With mBuf
            ...
            mRS1.Fields("SanrenFlag").Value = .SanrenpukuFlag '' 発売フラグ　3連複
            mRS1.Fields("TotalHyosuSanren").Value = .TotalHyosuSanrenpuku '' 3連複票数合計
        End With

        With mBuf.id
            System.Diagnostics.Debug.WriteLine("Insert ODDS_SANREN_HEAD : " & .Year & .MonthDay & .JyoCD & .Kaiji & .Nichiji & .RaceNum)
        End With ' id

        mRS1.Update()

        ' ODDS_SANREN (オッズ_3連複)
        If mBuf.SanrenpukuFlag <> "0" Then
            For i = 0 To 815
                If mBuf.OddsSanrenInfo(i).Kumi <> "      " Then
                    ...
                    With mBuf.id
                        System.Diagnostics.Debug.WriteLine("Insert ODDS_SANREN : " & .Year & .MonthDay & .JyoCD & .Kaiji & .Nichiji & .RaceNum & mBuf.OddsSanrenInfo(i).Kumi)
                    End With ' id
                    mRS2.Update()
                End If
            Next i
...
ErrorHandler:
    ...
    Public Function UpdateDB(ByRef strMakeDate As String) As Boolean
        ...
        ' ODDS_SANREN_HEAD (オッズ_3連複_ヘッダ)
        strSql = "UPDATE ODDS_SANREN_HEAD SET "
        With mBuf
            With .head
            ...
            strSql = strSql & SS & "SanrenFlag" & SE & "='" & Replace(.SanrenpukuFlag, "'", "''") & "'," '' 発売フラグ　3連複
            strSql = strSql & SS & "TotalHyosuSanren" & SE & "='" & Replace(.TotalHyosuSanrenpuku, "'", "''") & "'" '' 3連複票数合計
            With .id
        ...
        For i = 0 To 815
            ...
            With mBuf
                ...
                With .id
                    ...
                    strSql = strSql & " AND " & SS & "Nichiji" & SE & "='" & Replace(.Nichiji, "'", "''") & "'"
                    strSql = strSql & " AND " & SS & "RaceNum" & SE & "='" & Replace(.RaceNum, "'", "''") & "'"
                    strSql = strSql & " AND " & SS & "Kumi" & SE & "='" & Replace(mBuf.OddsSanrenInfo(i).Kumi, "'", "''") & "'"
                    strSql = strSql & " AND " & SS & "MakeDate" & SE & " <= '" & Replace(strMakeDate, "'", "''") & "'"
                End With ' id
            End With ' mbuf

            With mBuf.id
                System.Diagnostics.Debug.WriteLine("UPDATE ODDS_SANREN : " & .Year & .MonthDay & .JyoCD & .Kaiji & .Nichiji & .RaceNum & mBuf.OddsSanrenInfo(i).Kumi)
            End With ' id
            gCon.Execute(strSql)
        Next i
