...
Friend Class clsImportBR
	' @(h) clsReadBR.cls
	' @(s)
	' JVData "BR" データベース登録クラス
	'
	
	Private mBuf As JV_BR_BREEDER '' 生産者マスタ構造体
...
ErrorHandler:
    ...
    Private Function InsertDB() As Boolean
        ...
        With mBuf
            ...
            mRS.Fields("BreederName_Co").Value = .BreederName_Co '' 生産者名(法人格有)
            mRS.Fields("BreederName").Value = .BreederName '' 生産者名(法人格無)
            mRS.Fields("BreederNameKana").Value = .BreederNameKana '' 生産者名半角カナ
            mRS.Fields("BreederNameEng").Value = .BreederNameEng '' 生産者名欧字
            mRS.Fields("Address").Value = .Address '' 生産者住所自治省名
            For i = 0 To 1
                With .HonRuikei(i)
                    If i = 0 Then s1 = "H"
                    If i = 1 Then s1 = "R"
        ...
        With mBuf
            System.Diagnostics.Debug.WriteLine("INSERT SEISAN : " & .BreederCode)
        End With ' mBuf
...
ErrorHandler:
    ...
    Public Function UpdateDB(ByVal strMakeDate As String) As Boolean
        ...
        With mBuf
            ...
            strSql = strSql & SS & "BreederName_Co" & SE & "='" & Replace(.BreederName_Co, "'", "''") & "'," '' 生産者名（法人格有）
            strSql = strSql & SS & "BreederName" & SE & "='" & Replace(.BreederName, "'", "''") & "'," '' 生産者名（法人格無）
            strSql = strSql & SS & "BreederNameKana" & SE & "='" & Replace(.BreederNameKana, "'", "''") & "'," '' 生産者名半角カナ
            strSql = strSql & SS & "BreederNameEng" & SE & "='" & Replace(.BreederNameEng, "'", "''") & "'," '' 生産者名欧字
            strSql = strSql & SS & "Address" & SE & "='" & Replace(.Address, "'", "''") & "'," '' 生産者住所自治省名
            With .HonRuikei(0)
            ...
            strSql = strSql & " WHERE " & SS & "BreederCode" & SE & "='" & Replace(.BreederCode, "'", "''") & "'"
            strSql = strSql & " AND " & SS & "MakeDate" & SE & " <= '" & Replace(strMakeDate, "'", "''") & "'"
        End With ' mBuf

        With mBuf
            System.Diagnostics.Debug.WriteLine("UPDATE SEISAN : " & .BreederCode)
        End With ' mBuf
