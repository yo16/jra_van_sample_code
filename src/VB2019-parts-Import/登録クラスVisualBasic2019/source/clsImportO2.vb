...
Friend Class clsImportO2
	' @(h) clsReadO2.cls
	' @(s)
	' JVData "O2" データベース登録クラス
	'
	
	Private mBuf As JV_O2_ODDS_UMAREN '' オッズ（馬連）構造体
    ...
    Private Sub Class_Initialize_Renamed()
        ...
        'レコードセットオープン
        strSql = "SELECT * FROM ODDS_UMAREN_HEAD"
        ...
        strSql = "SELECT * FROM ODDS_UMAREN"
...
ErrorHandler:
    ...
    Public Function InsertDB() As Boolean
        ...
        With mBuf
            ...
            mRS1.Fields("UmarenFlag").Value = .UmarenFlag '' 発売フラグ　馬連
            mRS1.Fields("TotalHyosuUmaren").Value = .TotalHyosuUmaren '' 馬連票数合計
        End With

        With mBuf.id
            System.Diagnostics.Debug.WriteLine("Insert ODDS_UMAREN_HEAD : " & .Year & .MonthDay & .JyoCD & .Kaiji & .Nichiji & .RaceNum)
        End With ' id
        mRS1.Update()


        ' ODDS_UMAREN (オッズ_馬連)
        If mBuf.UmarenFlag <> "0" Then
            For i = 0 To 152
                If mBuf.OddsUmarenInfo(i).Kumi <> "    " Then
                    mRS2.AddNew()
                    With mBuf
                        With .head
                            With .MakeDate
                    ...
                    End With
                    With mBuf.id
                        System.Diagnostics.Debug.WriteLine("Insert ODDS_UMAREN : " & .Year & .MonthDay & .JyoCD & .Kaiji & .Nichiji & .RaceNum & mBuf.OddsUmarenInfo(i).Kumi)
                    End With ' id
...
ErrorHandler:
    ...
    Public Function UpdateDB(ByVal strMakeDate As String) As Boolean
        ...
        ' ODDS_UMAREN_HEAD (オッズ_馬連_ヘッダ)
        strSql = "UPDATE ODDS_UMAREN_HEAD SET "
        With mBuf
            With .head
            ...
            strSql = strSql & SS & "UmarenFlag" & SE & "='" & Replace(.UmarenFlag, "'", "''") & "'," '' 発売フラグ　馬連
            strSql = strSql & SS & "TotalHyosuUmaren" & SE & "='" & Replace(.TotalHyosuUmaren, "'", "''") & "'," '' 馬連票数合計
        ...
        End With
        With mBuf.id
            System.Diagnostics.Debug.WriteLine("UPDATE ODDS_UMAREN_HEAD : " & .Year & .MonthDay & .JyoCD & .Kaiji & .Nichiji & .RaceNum)
        End With ' id
        gCon.Execute(strSql)

        ' ODDS_UMAREN (オッズ_馬連)
        For i = 0 To 152
            strSql = "UPDATE ODDS_UMAREN SET "
            With mBuf
                strSql = strSql & SS & "MakeDate" & SE & "= '" & Replace(strMakeDate, "'", "''") & "'," '' 年月日
                With .id
                    strSql = strSql & SS & "Year" & SE & "='" & Replace(.Year, "'", "''") & "'," '' 開催年
                ...
                With .id
                    ...
                    strSql = strSql & " AND " & SS & "Nichiji" & SE & "='" & Replace(.Nichiji, "'", "''") & "'"
                    strSql = strSql & " AND " & SS & "RaceNum" & SE & "='" & Replace(.RaceNum, "'", "''") & "'"
                    strSql = strSql & " AND " & SS & "Kumi" & SE & "='" & Replace(mBuf.OddsUmarenInfo(i).Kumi, "'", "''") & "'"
                    strSql = strSql & " AND " & SS & "MakeDate" & SE & " <= '" & Replace(strMakeDate, "'", "''") & "'"
                End With ' id
            End With

            With mBuf.id
                System.Diagnostics.Debug.WriteLine("UPDATE ODDS_UMAREN : " & .Year & .MonthDay & .JyoCD & .Kaiji & .Nichiji & .RaceNum & mBuf.OddsUmarenInfo(i).Kumi)
            End With ' id
