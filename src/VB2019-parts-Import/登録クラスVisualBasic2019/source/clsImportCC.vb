...
Friend Class clsImportCC
	' @(h) clsReadCC.cls
	' @(s)
	' JVData "CC" データベース登録クラス
	'
	
	Private mBuf As JV_CC_INFO '' コース変更構造体
...
ErrorHandler:
    ...
    Public Function InsertDB() As Boolean
        ...
        With mBuf
            ...
            End With ' HappyoTime
            With .CCInfoAfter
                mRS.Fields("AtoKyori").Value = .Kyori '' 距離
                mRS.Fields("AtoTruckCD").Value = .TruckCD '' トラックコード
            End With ' CCInfoAfter
            With .CCInfoBefore
                mRS.Fields("MaeKyori").Value = .Kyori '' 距離
                mRS.Fields("MaeTruckCD").Value = .TruckCD '' トラックコード
            End With ' CCInfoBefore
            mRS.Fields("JiyuCD").Value = .JiyuCD '' 事由コード

        End With
        With mBuf.id
            System.Diagnostics.Debug.WriteLine("Insert COURSE_CHANGE : " & .Year & .MonthDay & .JyoCD & .Kaiji & .Nichiji & .RaceNum)
        End With ' id
...
ErrorHandler:
    ...
    Public Function UpdateDB(ByVal strMakeDate As String) As Boolean
        ...
        strSql = "UPDATE COURSE_CHANGE SET "
        With mBuf
            With .head
            ...
            End With ' HappyoTime
            With .CCInfoAfter
                strSql = strSql & SS & "AtoKyori" & SE & "='" & Replace(.Kyori, "'", "''") & "'," '' 距離
                strSql = strSql & SS & "AtoTruckCD" & SE & "='" & Replace(.TruckCd, "'", "''") & "'," '' トラックコード
            End With ' CCInfoAfter
            With .CCInfoBefore
                strSql = strSql & SS & "MaeKyori" & SE & "='" & Replace(.Kyori, "'", "''") & "'," '' 距離
                strSql = strSql & SS & "MaeTruckCD" & SE & "='" & Replace(.TruckCd, "'", "''") & "'," '' トラックコード
            End With ' CCInfoBefore
            strSql = strSql & SS & "JiyuCD" & SE & "='" & Replace(.JiyuCd, "'", "''") & "'," '' 事由コード
        ...
        With mBuf.id
            System.Diagnostics.Debug.WriteLine("UPDATE COURSE_CHANGE : " & .Year & .MonthDay & .JyoCD & .Kaiji & .Nichiji & .RaceNum & mBuf.HappyoTime.Month & mBuf.HappyoTime.Day) '.Hour & .Minute は略
        End With ' id
