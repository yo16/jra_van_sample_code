...
Friend Class clsImportAV
	' @(h) clsReadBN.cls
	' @(s)
	' JVData "BN" データベース登録クラス
	'
	
	Private mBuf As JV_AV_INFO '' 出走取消・競争除外構造体
...
ErrorHandler:
    ...
    Public Function InsertDB() As Boolean
        ...
        With mBuf
            ...
            mRS.Fields("JiyuKubun").Value = .JiyuKubun '' 事由区分
        End With

        mRS.Update()

        With mBuf.id
            System.Diagnostics.Debug.WriteLine("INSERT TORIKESI_JYOGAI : " & .Year & .MonthDay & .JyoCD & .Kaiji & .Nichiji & .RaceNum & mBuf.Umaban)
        End With ' id
...
ErrorHandler:
    ...
    Public Function UpdateDB(ByVal strMakeDate As String) As Boolean
        ...
        strSql = "UPDATE TORIKESI_JYOGAI SET "
        With mBuf
            With .head
            ...
            strSql = strSql & SS & "JiyuKubun" & SE & "='" & Replace(.JiyuKubun, "'", "''") & "'," '' 事由区分
        ...
        With mBuf.id
            System.Diagnostics.Debug.WriteLine("UPDATE TORIKESI_JYOGAI : " & .Year & .MonthDay & .JyoCD & .Kaiji & .Nichiji & .RaceNum & mBuf.Umaban)
        End With ' id
